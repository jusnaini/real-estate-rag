...
LLM Zoomcamp 2026 capstone project. A Retrieval-Augmented Generation assistant that helps first-time property buyers in Malaysia get step-by-step guidance on buying property, fees, insurance, and legal costs.
...
**Real Estate RAG Assistant** is an end-to-end **Retrieval-Augmented Generation (RAG)** application developed as the capstone project for **LLM Zoomcamp 2026**.

The project demonstrates how a domain-specific AI assistant can be built by combining document ingestion, knowledge base construction, retrieval, prompt engineering, LLM generation, evaluation, monitoring, and a conversational interface into a complete RAG workflow.

Rather than relying solely on a Large Language Model's internal knowledge, the assistant retrieves relevant information from a curated knowledge base before generating responses. This grounding process improves answer transparency, reduces hallucinations, enables source attribution, and allows the knowledge base to be updated independently of the underlying language model without requiring model retraining.

Malaysian residential property purchasing is selected as the demonstration domain because it contains fragmented information spread across property guides, financial resources, legal references, insurance articles, and online calculators. This makes it an ideal real-world use case for illustrating how Retrieval-Augmented Generation can consolidate scattered knowledge into a single conversational interface.
...
Many real-world knowledge domains consist of information distributed across multiple documents, websites, and reference materials. While modern Large Language Models provide impressive conversational capabilities, they cannot guarantee that responses are grounded in the most relevant, up-to-date, or organization-approved knowledge sources.

This challenge is particularly evident in the Malaysian property purchasing journey, where information related to financing, legal fees, stamp duty, insurance, documentation, and purchasing procedures is scattered across numerous trusted resources. Users often need to navigate multiple websites, compare information from different sources, and interpret financial or legal terminology before making informed decisions.

Building a reliable AI assistant for this domain therefore requires more than simply prompting a language model. It requires a system capable of ingesting external knowledge, retrieving relevant context, generating grounded responses, and evaluating retrieval and answer quality systematically.

---

## Solution

This project implements a complete Retrieval-Augmented Generation (RAG) pipeline that transforms curated Malaysian real estate resources into a searchable knowledge base capable of supporting natural language question answering.

The implementation covers the complete lifecycle of a modern RAG application, including:
...
- *What costs should I prepare before signing the Sales and Purchase Agreement (SPA)?*

Instead of answering solely from the model's internal knowledge, the system retrieves the most relevant document chunks from the curated knowledge base and uses them as context for response generation. Every answer is accompanied by references to the original sources, allowing users to verify the information while demonstrating the grounding capability of Retrieval-Augmented Generation.

---

## Scope

This project is implemented as a **Minimum Viable Product (MVP)** focusing on the Malaysian residential property purchasing process.

The initial knowledge base covers four primary areas:

- **Buying Process** — purchasing workflow, required documents, Sales and Purchase Agreement (SPA), and ownership transfer.
- **Financing** — home loan application, Debt Service Ratio (DSR), Loan-to-Value (LTV), affordability, and financing concepts.
- **Legal & Transaction Costs** — stamp duty, legal fees, Memorandum of Transfer (MOT), and related purchasing expenses.
- **Insurance** — MRTA, MLTA, fire insurance, and homeowner protection.

To simplify development for the MVP, the knowledge base is constructed from a curated collection of reputable Malaysian property education websites, financial guidance articles, and legal fee references rather than comprehensive government publications. The ingestion pipeline is designed to be extensible, allowing additional sources to be incorporated with minimal changes to the overall architecture.

---

## Technical Highlights

This project demonstrates several practical concepts covered throughout LLM Zoomcamp, including:

- End-to-end Retrieval-Augmented Generation (RAG) implementation
- Modular document ingestion and preprocessing pipeline
- Configurable retrieval architecture
- Prompt engineering for grounded answer generation
- Multi-provider LLM integration through the OpenAI SDK compatibility layer
- Retrieval evaluation using Hit Rate and Mean Reciprocal Rank (MRR)
- Answer evaluation using LLM-as-a-Judge
- Application monitoring and interaction logging
- Modular architecture designed for future extensibility

Rather than being a domain-specific chatbot, this project serves as a reusable blueprint for developing maintainable, explainable, and measurable RAG systems that can be adapted to different industries by replacing the underlying knowledge corpus.
